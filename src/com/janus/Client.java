...
import com.janus.content.InterfaceNPC;
...
public class Client extends RSApplet {
    ...
    private static Stopwatch messageDelay;
    ...
    public void loadSettings() {
        ...
        try {
            ...
            options.put("one_click", in.readByte() == 1);
    ...
    public void saveSettings() {
        try {
            ...
            out.write(getOption("one_click") ? 1 : 0);
    ...
    private final String[][] PLRCOMMANDS = new String[][]{{"Enter character", "hangman"},
            {"Enter house owners's name", "popE158"}};
...
//								System.out.println("Suppose to draw here: "+playerRights+"");
    ...
    public void drawChannelButtons(int xPosOffset, int yPosOffset) {
        ...
        smallText.drawCenteredText(0xffffff, 459 + xPosOffset, "Janus.rip", 157 + yPosOffset, true);
        ...
        smallText.drawCenteredText(textColor[clanChatMode], CHANNELBUTTON_X[4] + 28 + xPosOffset, "All",
                163 + yPosOffset, true);
        //smallText.drawCenteredText(textColor[tradeMode], CHANNELBUTTON_X[5] + 28 + xPosOffset, text[tradeMode], 163 + yPosOffset, true);
    }
        ...
        if (super.clickMode3 == 1) {
            if (super.saveClickX >= x[0] && super.saveClickX <= x[0] + 56 && super.saveClickY >= clientHeight - 23
                ...
                if (clientSize != 0) {
                    ...
                    && super.saveClickY <= clientHeight) {
                launchURL("https://www.janus.rip");
                pushMessage("We've attempted to open www.janus.rip for you.", 0, "");
            }
            if (!showChat) {
                cButtonCPos = -1;
            }
    ...
    public void rightClickMapArea() {
        ...
        if (mouseInRegion(clientSize == 0 ? clientWidth - 58 : getOrbX(2), getOrbY(2),
                (clientSize == 0 ? clientWidth - 58 : getOrbX(2)) + 57, getOrbY(2) + 34)) {
            menuActionName[menuActionRow] = "AFK";
        ...
        try {
            ...
            if (buttonclicked && inputDialogState == 3) {
                ...
                if (amountOrNameInput.length() == 0) {
                    chatTextDrawingArea.drawCenteredText(0xA05A00, 259, "Item Price Checker", 30 + yPosOffset,
                            false);
    ...
    private void buildInterfaceMenu(int interfaceX, RSInterface class9, int mouseX, int interfaceY, int mouseY,
        ...
        for (int frameID = 0; frameID < totalChildrens; frameID++) {
            ...
            if (child.type == 0) {
                ...
                if (child.atActionType == 1 && mouseX >= realX && mouseY >= realY && mouseX < realX + child.width
                        && mouseY < realY + child.height) {
    ...
    private void mainGameProcessor() throws ClassNotFoundException, InstantiationException, IllegalAccessException,
        ...
        for (int j = 0; j < 35; j++) {//SWITCH IMPROVEMENTS. THIS CHECKS 35PACKETS PER TICK NOW!
            if (!parsePacket()) {
                break;
            }
        }
	...
	/*	if (super.mouseX >= 0 && super.mouseX <= 1 && super.mouseY >= 0 && super.mouseY <= 1) {
        ...
        if (!isLoading) {
            if (super.mouseX >= 390 && super.mouseX <= 514 && super.mouseY >= 377 && super.mouseY <= 420) {
                loginButtonHover = handCursor = true;
            } else if (super.mouseX >= 266 && super.mouseX <= 482) {
                if (super.mouseY >= 262 && super.mouseY <= 295) {
                    input1Hover = textCursor = true;
                } else if (super.mouseY >= 336 && super.mouseY <= 357) {
                    input2Hover = textCursor = true;
                }
            }
            if (super.mouseX >= 335 && super.mouseX <= 379 && super.mouseY >= 383 && super.mouseY <= 406) {//MOVE LOGIN BUTTONS
    ...
    private void doAction(int i) {
        ...
        if (l == 696) {
            viewRotation = 0;
            //anInt1184 = 120;
            minimapRotation = 0; //Minimap rotation fix
        ...
        if (l == 169) {
            ...
            switch (interfaceId) {
                ...
                case 35590:
                    ...
                    return;
                case 35619:
                    options.put("one_click", !getOption("one_click"));
                    saveSettings();
                    return;
            }
        }
    ...
    private void build3dScreenMenu() {
        if (itemSelected == 0 && spellSelected == 0) {
            ...
            if (intersectingWidget() != null) {
                buildInterfaceMenu(4, intersectingWidget(), super.mouseX, 4, super.mouseY, 0); //whatis this do u know? No idea man
            } else {
    ...
    public static Stopwatch getMessageDelay() {
        return messageDelay;
    }
    ...
    private void drawFriendsListOrWelcomeScreen(RSInterface rsi) {
        ...
        if (j == 3291) {
            ...
            npcDisplay = NPCDef.forID(rsInterface.npcDisplay);
...
//		System.out.println(title);
    ...
    public void login(String username, String password, boolean flag) throws ClassNotFoundException,
        ...
        try {
            ...
            if (username.contains("  ")) {
                loginMessages = new String[]{"Your username cannot contain two spaces."};
                return;
            }
            ...
            if (loginCode == 0) {
				...
				/*System.out.print("Mac:"+GeneratedValues.getValue());
				System.out.print("UID?:"+GeneratedValues.generatedValue);*/
            ...
            if (loginCode == 6) {
                loginMessages = new String[]{"Janus is currently being updated.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 7) {
                loginMessages = new String[]{"Janus is currently busy.", "Please try again."};
                return;
            }
            if (loginCode == 8) {
                loginMessages = new String[]{"Janus is down.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 9) {
            ...
            if (loginCode == 23) {
                loginMessages = new String[]{"Janus is currently being launched.",
                        "Please try again in 60 seconds.."};
                return;
            }
            if (loginCode == 27) {
            ...
            if (loginCode == 30) {
                loginMessages = new String[]{"Janus has been updated!", "Downloading updated version..."};
                launchURL("https://www.janus.rip/downloads/Janus.jar");
                return;
            }
    ...
    public void load() {
        try {
            ...
            //repackCacheIndex(2); // Animation
            //repackCacheIndex(3); // Sounds
            //repackCacheIndex(4); // Map
    ...
    private void showErrorScreen() {
        ...
        if (loadingError) {
            ...
            int k = 35;
            g.drawString("Sorry, an error has occured whilst loading Janus", 30, k);
            k += 50;
        ...
        if (genericLoadingError) {
            ...
            g.drawString("To play Janus make sure you play from", 50, 100);
            g.drawString("https://www.janus.rip/downloads/Janus.jar", 50, 150);
        }
    }
                    ...
                    if (child.type == 2) {
                        ...
                        if(child.widgetActive) {
                            if (mouseX >= realX && mouseY >= realY
                                    && mouseX < realX + child.width && mouseY < realY + child.height) {
                                child.sprite2.drawSprite(realX, realY);
                            } else {
                                child.sprite1.drawSprite(realX, realY);
                            }
                            if (child.centerText)
                                chatTextDrawingArea.drawCenteredText(child.disabledColor, realX + child.xOffset, child.message, realY + child.yOffset, true);
                            else
                                child.getFont().drawBasicString(child.message, realX + 5 + child.xOffset, realY + child.yOffset, child.disabledColor, child.enabledColor);
                        }
    ...
    private void connectToUpdateServer() {
        ...
        while (expectedCRCs[8] == 0) {
            ...
            setLoadingText(10, "Connecting to Janus...");
            try {
    ...
    public void drawLoginScreen() {
        ...
        if (loginButtonHover) {
            SpriteLoader.sprites[596].drawAdvancedSprite(396, 383);
        }

        if (rememberMe)
//			335 && super.mouseX <= 379 && super.mouseY >= 353 && super.mouseY <= 376
            SpriteLoader.sprites[/*rememberMe ? */600/* : 599*/].drawAdvancedSprite(351, 378);

        if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
            chatTextDrawingArea.drawRegularText(true, 277, 16777215, myUsername + "|", 289);
        } else {
            chatTextDrawingArea.drawRegularText(true, 277, 16777215, myUsername, 289);
        }

        if (loginScreenCursorPos == 1 && loopCycle % 45 < 10) {
            chatTextDrawingArea.drawRegularText(true, 277, 16777215, getStars(myPassword) + "|", 354);
        } else {
            chatTextDrawingArea.drawRegularText(true, 277, 16777215, getStars(myPassword), 354);
        }
...
//				}
    ...
    private boolean parsePacket() {
        ...
        try {
            ...
            anInt841 = opCode;
            switch (opCode) {
                case 131://DISCORD PRESENCE - old string method: getInputBuffer().getString()
                ...
                case 139://DISCORD PRESENCE - old string method: getInputBuffer().getString()
                    String hoverBigImageText = inStream.readString();
                    RICH_PRESENCE.updateBigHoverText(hoverBigImageText);
                    opCode = -1;
                    return true;

                case 129://DISCORD PRESENCE - old string method: getInputBuffer().getString()
                    String hoverSmallImageText = inStream.readString();
                    RICH_PRESENCE.updateSmallHoverText(hoverSmallImageText);
                    opCode = -1;
                    return true;
                ...
                case 190:
                    ...
                    npcOnInterface.npcDisplay = npcId; // k..... wtf
                    npcOnInterface.modelZoom = npcZoom;
                    opCode = -1;
                    return true;

                case 210:
                    int widgId = inStream.readShort();
                    int receivedActive = inStream.readShort();
                ...
                case 253:
                    ...
                    if (s.endsWith(":tradereq:")) {
                        ...
                        if (!flag2 && s3.length() >= 2) {
                            pushMessage("sent you a trade request. Click here to accept.", 4, s3);
                        }
                    } else if (s.endsWith("::")) {
    ...
    public static final RichPresence RICH_PRESENCE = new RichPresence();//DISCORD PRESENCE

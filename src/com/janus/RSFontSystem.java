...
public class RSFontSystem extends DrawingArea {
    ...
    public static String handleOldSyntax(String text) {
        ...
        if (text != null && SMILIES_TOGGLED && !text.contains(":store")) {
            ...
            text = replace(text, ":Mod", "<img=1>");
            text = replace(text, ":mod", "<img=1>");
            text = replace(text, "cash", "<img=17>");
            text = replace(text, "Cash", "<img=17>");
            text = replace(text, "money", "<img=17>");
            text = replace(text, ":youtube", "<img=11>");
            text = replace(text, ":yt", "<img=11>");
            text = replace(text, ":yt", "<img=11>");
            text = replace(text, ":admin", "<img=2>");
            text = replace(text, ":Admin", "<img=2>");
            text = replace(text, ":Owner", "<img=3>");
            text = replace(text, ":owner", "<img=3>");
            text = replace(text, ":developer", "<img=4>");
            text = replace(text, ":Developer", "<img=4>");
            text = replace(text, ":support", "<img=10>");
            text = replace(text, ":Support", "<img=10>");
            text = replace(text, ":ironman", "<img=12>");
            text = replace(text, ":Ironman", "<img=12>");
            text = replace(text, ":communitymanager", "<img=31>");
            text = replace(text, ":Communitymanager", "<img=31>");
            text = replace(text, ";)", "<img=27>");
            text = replace(text, ";]", "<img=27>");
            text = replace(text, ":/", "<img=30>");
            text = replace(text, ":\\", "<img=30>");
            text = replace(text, "\\:", "<img=30>");
            text = replace(text, ":bad", "<img=21>");
            text = replace(text, ":Bad", "<img=21>");
            text = replace(text, ":P", "<img=23>");
            text = replace(text, ":D", "<img=25>");
            text = replace(text, "=D", "<img=25>");
            text = replace(text, "=d", "<img=25>");
